...
public class HayvonlarActivity extends  AppCompatActivity  { 
	
	
	private String json_hayvonlar = "";
	
	private ArrayList<HashMap<String, Object>> listmap_hayvonlar = new ArrayList<>();
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.hayvonlar);
	...
	private void initializeLogic() {
		...
		try { 
			...
			this.getAssets().open("hayvonlar.json");
			int size = is.available();
			byte[] buffer = new byte[size];
			is.read(buffer);
			is.close();
			json_hayvonlar = new String(buffer, "UTF-8");
			listmap_hayvonlar = new Gson().fromJson(json_hayvonlar, new TypeToken<ArrayList<HashMap<String, Object>>>(){}.getType());
			listview1.setAdapter(new Listview1Adapter(listmap_hayvonlar));
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			textview_nom.setText(listmap_hayvonlar.get((int)_position).get("nom").toString());
			textview_malumot.setText(listmap_hayvonlar.get((int)_position).get("malumot").toString());
			linear1.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View _view) {
					view_intent.setClass(getApplicationContext(), ViewActivity.class);
					view_intent.putExtra("rasm_key", listmap_hayvonlar.get((int)_position).get("rasm").toString());
					view_intent.putExtra("nom_key", listmap_hayvonlar.get((int)_position).get("nom").toString());
					view_intent.putExtra("malumot_key", listmap_hayvonlar.get((int)_position).get("malumot").toString());
